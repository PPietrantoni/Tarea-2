...
int main () {
    ...
    Map * albums = createMap(stringHash,stringEqual);
        ...
        if(op==1){
            ...
            if(fp==NULL){
                ...
                while (fgets (linea, 1023, fp) != NULL){
                    ...
                    if(searchMap(albums,c->album)!=NULL){
                        album *Al = crearAlbum();
                        Al->capac++;
                        list_push_back(Al->songAl,c->nombre);
                        insertMap(albums,c->album,Al):
                    }
                    else{
                        album *AuxAl=crearAlbum();
                        AuxAl=searchMap(albums,c->album);
                        AuxAl->capac++;
                        list_push_back(AuxAl->songAl,c->nombre);
                        insertMap(albums,c->album,AuxAl);
                    }


                    if(searchMap(artistas,c->artista)!=NULL){
                        artist *Ar =crearArtista();
                        Ar->capac++;
                        list_push_back(Ar->songAr,c->nombre);
                        insertMap(artistas,c->album,Ar):
                    }
                    else{
                        artist *AuxAr=crearArtista();
                        AuxAr=searchMap(artistas,c->artista);
                        AuxAr->capac++;
                        list_push_back(AuxAr->songAr,c->nombre);
                        insertMap(artistas,c->artista,AuxAr);
                    }
...
artist *crearArtista(){
    ...
    artist *artista=malloc(sizeof(artist));
    list_create_empty(artista->songAr);
    artista->capac=0;
...
album *crearAlbum(){
    album *Album=malloc(sizeof(artist));
    list_create_empty(Artista->songAl);
    Album->capac=0;
